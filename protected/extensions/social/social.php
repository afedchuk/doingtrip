...
class social extends CInputWidget
{
	...
	public $networks = array(
		'twitter'=>array(
			'data-via'=>'https://twitter.com/house4trip', //http://twitter.com/#!/YourPageAccount if exists else leave empty
		), 
		'googleplusone'=>array(
			"size"=>"medium",
			"annotation"=>"bubble",
		...
		'facebook'=>array(
			'href'=>'https://www.facebook.com/house4trip',//asociate your page http://www.facebook.com/page 
	...
	private function renderSocial(){
		...
		foreach($this->networks as $network => $params) {
			...
			$arr[$network] = $this->render($network, array(), true);
		}
		echo $this->render('social', array('rendered'=>$rendered, 'soc' => $arr));
	}
}
