...
class Images extends ParentModel {
	...
	public static function createFullSize($image){
		...
		if(!param('useWatermark')){
			...
			if(param('watermarkType') == ImageSettings::WATERMARK_FILE){
				...
				$img->thumb(param('maxImageWidth'), param('maxImageHeight'), self::KEEP_PHOTO_PROPORTIONAL);
	...
	public static function createThumb($image, $width, $height, $adaptive){
	...
	public static function getThumbUrl($image, $width=0, $height=0, $adaptive=false){
		if($image['file_name_modified']){
	...
	public static function getMainThumb($width, $height, $images, $id = null, $adaptive=false){
		...
		$image = null;
		if($id !== null){
			$image = Yii::app()->db->createCommand(array(
			    'select' => array('comment', 'file_name', 'file_name_modified', 'id', 'id_object'),
			    'from' => '{{images}}',
			    'where' => 'id_object=:id_object AND is_main=1',
			    'params' => array(':id_object'=>$id),
			))->queryRow();
		}
		...
		if($image){
			...
			$return['thumbUrl'] = self::returnEmptyImgUrl($width, $height, $adaptive);
		}
	...
	public static function getMaxSizeLimit(){
		...
		return min($min, param('maxImgFileSize', 15 * 1024 * 1024));
	}
