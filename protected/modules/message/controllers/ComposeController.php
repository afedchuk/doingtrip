...
class ComposeController extends ModuleUserController
{
	...
	public function actionCompose($id = null) {
		...
		if (Yii::app()->request->getPost('Message')) {
			...
			if(Yii::app()->request->getPost('Message')['adminLetter'] == 1) {
				$admin = call_user_func(array(call_user_func(array(Yii::app()->getModule('message')->userModel, 'model')), 'findByAttributes'), array('email' => param('adminEmail')));
				$message->receiver_id = $admin->id;
				$receiverName = implode(' ',array($admin->firstname, $admin->lastname));
			} else
