...
$columns = array(
    ...
    array(
     ...
     array(
        ...
        'type' => 'raw',
        'value' => 'isset($data->description->title) ? CHtml::link(CHtml::encode($data->description->title),Apartment::model()->getUrl($data->id, $data->description->title, City::getCityName($data->city_id))) : "undefined"',
        'sortable' => false,
    ),
    array(
        'name' => 'city_id',
...
$columns[] = array(
    ...
    'buttons' => array(
        ...
        'active' => array(
            ...
            'icon'=>'eye-open',
            'url'=>'Yii::app()->createUrl("user/profile/activateadmin", array("id"=>$data->id))',
            'visible'=>'($data->active==0 || $data->active==2)?true:false;'
        ),
        'update' => array(
